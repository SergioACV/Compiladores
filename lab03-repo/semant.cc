...
{
    // Iterador explícito de map<Symbol, Class_>
    std::map<Symbol, Class_>::const_iterator it;
    ...
    for (it = this->class_lookup.begin(); it != this->class_lookup.end(); ++it)
    {
        ...
        // Crear entrada para el padre si no existe
        ...
        // Agregar hijo a la lista del padre
...
void build_attribute_scopes(Class_ current_class) {
    ...
    for (it = attrs.begin(); it != attrs.end(); ++it) {
        attr_class* attr_definition = it->second;
...
void process_attr(Class_ current_class, attr_class* attr) {
    if (get_class_attr(current_class->get_name(), attr->get_name()) != NULL)
    {
...
void type_check(Class_ next_class) {
    ...
    std::map<Symbol, method_class*>::iterator m_it;
    for (m_it = current_class_methods.begin(); m_it != current_class_methods.end(); ++m_it) {
        process_method(next_class, m_it->second, m_it->second);
    }

   
    std::map<Symbol, attr_class*>::iterator a_it;
    for (a_it = current_class_attrs.begin(); a_it != current_class_attrs.end(); ++a_it) {
        Symbol parent_type_name = class_table->get_parent_type_of(current_class_name);
        Class_ parent_definition = class_table->class_lookup[parent_type_name];
        process_attr(parent_definition, a_it->second);
    }

    for (a_it = current_class_attrs.begin(); a_it != current_class_attrs.end(); ++a_it) {
        a_it->second->type_check();
    }

    for (m_it = current_class_methods.begin(); m_it != current_class_methods.end(); ++m_it) {
        m_it->second->type_check();
    }
...
Symbol attr_class::type_check() {
    ...
    if (dynamic_cast<const no_expr_class*>(init_expr) != NULL)
        return this->get_type();
...
void program_class::semant()
{
   ...
   std::map<Symbol, Class_>::iterator it;
    ...
    for (it = class_table->class_lookup.begin();
        it != class_table->class_lookup.end();
        ++it)
    {
        register_class_and_its_methods(it->second);
    }
